...
data = {
    ...
    "date": pd.date_range(start="2024-01-01", periods=num_rows, freq="min"),  # Corrected: 'T' → 'min'
    ...
    "in_game_purchases": np.random.choice([0, 0, 0, 1.99, 4.99, 9.99, 49.99], num_rows, 
...
# Introducing issues in the dataset 🔥

# 🔴 1. Add missing values in some columns (2% missing values)
for col in ["session_length", "frame_rate", "cpi"]:
    df.loc[df.sample(frac=0.02).index, col] = np.nan  

# 🔴 2. Add extreme values (outliers)
outlier_indices = df.sample(frac=0.005).index  # 0.5% extreme values
df.loc[outlier_indices, "session_length"] = np.random.randint(500, 2000, len(outlier_indices))  
df.loc[outlier_indices, "frame_rate"] = np.random.uniform(5, 15, len(outlier_indices))  
df.loc[outlier_indices, "cpi"] = np.random.uniform(50, 200, len(outlier_indices))  

# 🔴 3. Add duplicate records (1% duplicates)
duplicated_rows = df.sample(frac=0.01)
df = pd.concat([df, duplicated_rows]).reset_index(drop=True)  # Reset index to avoid inconsistencies

# 🔴 4. Add formatting errors in the `acquisition_source` column
# Selecting 2% of the rows after data duplication
sample_indices = df.sample(frac=0.02).index  

# Creating corrupted values with the exact size of sample_indices
corrupted_values = np.random.choice(
    ["Ad$", "0rganic", "refferal", "AdS"], size=len(sample_indices), replace=True
)

# Updating the `acquisition_source` column with corrupted values
df.loc[sample_indices, "acquisition_source"] = corrupted_values

# 🔴 5. Adjust retention to keep it consistent
df["retention_day"] = df["retention_day"].apply(retention_probability)

# 🔴 6. Calculate ROAS (Return on Ad Spend) = (Total Revenue / Acquisition Cost) * 100
...
# Save as CSV in the GAMES_PATH directory
csv_file_path = os.path.join(games_path, "game_analytics_dataset.csv")
df.to_csv(csv_file_path, index=False)

print(f"🚀 Dataset 'game_analytics_dataset.csv' created and saved at: {csv_file_path}")
